...
Decide only from the supplied evidence. POSSIBLE_SUCCESS means the outcome appears likely achieved and must cite at least one concrete source message; it never means the Loop is closed. STILL_WAITING means the evidence still supports waiting. NEEDS_USER means the user must decide, provide information, or take a manual step. UNKNOWN means the evidence is ambiguous or insufficient. Never infer that silence proves completion. Keep rationale and userSummary short, calm, factual, and do not quote unnecessary private details. recommendedNextCheckAt is only for STILL_WAITING, as an ISO timestamp, and should be conservative (at least 12 hours from now).`;
