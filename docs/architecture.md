...
`agent_jobs` is PostgreSQL-backed and owns wake-ups. A worker claims one due job in a short transaction with `FOR UPDATE SKIP LOCKED`, assigns a 90-second lease, and increments its attempt counter. During Gmail and model work it renews that lease every 25 seconds using an update that requires the job ID, owner ID, RUNNING status, and exact lease ID. Failed renewal aborts the network signal and prevents token, evidence, retry, or result writes from the stale worker. Jobs carry the owner, Loop, monitoring generation, run time, idempotency key, lease, bounded `maxAttempts` (1–10), and result marker. A crash leaves the job reclaimable after its lease. Result application locks the job and Loop together, confirms the lease, owner, generation, enabled policy, and compatible state, then writes the immutable observation, Loop state, next job, and completed result marker in one transaction. Duplicate delivery therefore cannot apply an observation twice; a changed policy generation cancels stale work.

The worker does not hold a database transaction while decrypting a token, fetching Gmail, or calling the evaluator. Token refresh, evidence persistence, and result application use separate short lease-fenced transactions. It asks Gmail only for `threads/{linked conversation}`; it never runs a mailbox scan for a scheduled Loop. Every observed message keeps trace identity, dedupe key, timestamp, and direction. At most the 12 most recent normalized excerpts plus accepted provenance are staged for the active evaluation. After a decision, only accepted provenance and evidence cited by immutable Loop events retain sender, subject, normalized text, and date hints; other excerpts are cleared. Scan cleanup preserves both cited evidence and the bounded context of an actively running observation. The evaluator therefore receives a compact relevant window, Loop outcome/verification context, and the previous short observation—not an indefinitely accumulated thread. Its strict Zod output is validated against persisted evidence IDs before service application.

Deterministic handling comes first: disabled, closed, stale, and incompatible jobs cancel; a future expected window is deferred; no new message reschedules without an AI call; disconnected or revoked Gmail moves the Loop to NEEDS_USER. Semantic decisions map to WAITING, VERIFYING, or NEEDS_USER. No agent path can write CLOSED. Temporary source/model/storage failures retry with 30-second exponential backoff capped at one hour until that job’s persisted `maxAttempts` is exhausted. A permanent Gmail auth failure stops retries, clears the encrypted token through the existing connection status path, and surfaces NEEDS_USER. Repeated failures likewise surface NEEDS_USER with audit history.
